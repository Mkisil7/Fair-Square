          ...
          <div className="p-6 space-y-6">
            {scannedReceiptData ? (
              <div className="animate-in fade-in slide-in-from-bottom-4">
                <div className="flex justify-between items-center mb-6">
                  <button onClick={() => setScannedReceiptData(null)} className="flex items-center gap-1 text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition-colors font-medium">
                    <ChevronLeft className="w-5 h-5" /> Cancel Split
                  </button>
                </div>
                <AssignAndSplit
                  initialReceiptData={scannedReceiptData}
                  users={Object.keys(trip.memberNames).map(id => ({ id, name: trip.memberNames[id] }))}
                  groupId={trip.id}
                  uploadedBy={user.uid}
                  paidBy={user.uid}
              ...
              <>
                {/* Scanner Action Card */}
                <div className="bg-white dark:bg-zinc-900 border border-indigo-100 dark:border-indigo-900/30 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white">Split a Receipt</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Upload an image and let AI calculate the splits.</p>
                  </div>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  <button
                    ...
                    disabled={isScanning}
                    className="w-full sm:w-auto flex flex-none items-center justify-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-xl font-semibold shadow-md shadow-indigo-200 dark:shadow-none hover:bg-indigo-700 hover:shadow-lg transition-all disabled:opacity-50"
                  >
                    {isScanning ? (
                      <>
                ...
                {/* Group Ledger Component */}
                <GroupLedger
                  groupId={trip.id}
                  currentUserId={user.uid}
                  users={Object.keys(trip.memberNames).reduce((acc, id) => {
                    acc[id] = { id, name: trip.memberNames[id] };
                    return acc;
